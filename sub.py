...
class Subreddit:
	...
	def flairUser(sub, user, flair_text, css):
		...
		try:
			...
			return False
		print('Flaired user: ' + str(user)
			  + '\tFlair: ' + flair_text + '\tCSS:' + css)
		return True
	...
	def getUserInfo(sub, username):
		...
		comment_karma_counter = sub.makeDict(
			info_dict['comment_karma_counter'])
	...
	def updateSub(sub, sub_name):
		...
		str_config = reddit.subreddit(
			sub_name).wiki['InstaModSettings'].content_md
		...
		for user_info in currentDB:
			tdelta = current_time - \
				dateutil.parser.parse(user_info['analysis_time'])
			...
			if tdelta.days > exp_length:
				...
				currentDB.remove(
					find_stuff['username'] == user_info['username'])
			else:
		...
		for username in expiredDB:
			...
			if user != None:
				sub.expired_users.append(user)
		print('\tRead ' + str(len(sub.expired_users)) +
			  ' users from expired list')
		...
		print('\tRead ' + str(len(sub.flair_img)) +
			  ' users from flair image permission list\n')
